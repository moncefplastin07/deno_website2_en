...
const Checked = (): React.SFC<React.SVGProps<SVGSVGElement>> => (
  <svg
    ...
    <div id="pol">
      <div className="h-full mt-1 ml-1 drop-down-button">
        <button
          ...
          value="tedst"
          className=" px-3 py-2 block w-full leading-normal bg-white rounded-lg outline-none appearance-none mt-1"
        >
          <svg
            width="25"
            height="25"
      ...
      <div
        className={`absolute bg-white border border-gray-200 rounded-md ${
          isOpen ? "block" : "hidden"
        }`}
      >
        <button
          value="star_count"
          className="border-b border-gray-200 block px-5 py-3 w-full text-left hover:bg-gray-100 focus:outline-none focus:bg-gray-100 transition duration-150 ease-in-out"
          onClick={props.onChange}
        >
          {props.selected === "star_count" ? <Checked /> : ""}
          <span>Stars</span>
        </button>
        <button
          value="search_score"
          className="block px-5 py-3 w-full text-left hover:bg-gray-50 focus:outline-none focus:bg-gray-50 transition duration-150 ease-in-out"
          onClick={props.onChange}
        >
          <p>
            {props.selected === "search_score" ? <Checked /> : ""}Popularity
          </p>
        </button>
      </div>
    </div>
